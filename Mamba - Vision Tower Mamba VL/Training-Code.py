...
# Create a MultiModal Connector with input_dim=1000 (MambaVision logits) and output_dim=2560
...
optimizer = AdamW(
    list(mamba_llm.parameters()) + list(mmc.parameters()), 
    lr=5e-5, 
    weight_decay=0.01
)
...
# Function to validate the vision model's output (Optional)
def verify_vision_model_output(vision_model, device):
    with torch.no_grad():
        sample_image = torch.randn(1, 3, 224, 224).to(device)  # Dummy image tensor
        visual_output = vision_model(sample_image)
        print("Vision Model Output Keys:", visual_output.keys())

        if 'logits' in visual_output:
            logits = visual_output['logits']
            print("Logits Shape:", logits.shape)  # Should be [1, 1000]
        else:
            raise AttributeError("The vision model output does not contain 'logits'")

# Uncomment the following line to verify the vision model's output before training
# verify_vision_model_output(vision_model, device)
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch}/{epochs}"):
            ...
            visual_token = visual_output.unsqueeze(1).float()  # Ensure float32
            text_embeddings = mamba_llm.get_input_embeddings()(input_ids)  # Shape: [B, N, 2560]
            combined_embeddings = torch.cat([visual_token, text_embeddings], dim=1).float()  # Shape: [B, N+1, 2560]
        ...
        # Save checkpoint every 'save_every' epochs or at the end
